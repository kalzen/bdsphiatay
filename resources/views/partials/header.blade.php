<!-- Add a simple header check to debug -->
<script>
    console.log('Header is loaded');
</script>
<!-- Then the actual header content -->
<header id="header" class="header header-default style-absolute header-fixed">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <div class="sticky-area-wrap">
                    <div class="header-ctimate">
                        <div class="logo-header">
                            <a href="{{route('index')}}" title="Logo"><img class="logo-dark" src="{{asset('logo.png')}}" alt="images"></a>
                        </div>
                        <div class="mobile-button">
                            <span></span>
                        </div>
                        <div class="nav-wrap">
                            <nav id="mainnav" class="mainnav">
                                <ul class="menu">
                                    <li class="{{ request()->routeIs('index') ? 'current-menu-item' : '' }}">
                                        <a href="{{route('index')}}">Trang chủ</a>
                                    </li>
                                    <li class="{{ request()->routeIs('about') ? 'current-menu-item' : '' }}">
                                        <a href="{{route('about')}}">Giới thiệu</a>
                                    </li>
                                    <li class="{{ request()->is('du-an*') ? 'current-menu-item' : '' }}">
                                        <a href="{{route('product.index')}}">Dự án</a>
                                    </li>
                                    <li class="{{ request()->is('tin-tuc*') ? 'current-menu-item' : '' }}">
                                        <a href="{{route('post.list')}}">Tin tức</a>
                                    </li>
                                    <li class="{{ request()->routeIs('contact') ? 'current-menu-item' : '' }}">
                                        <a href="{{route('contact')}}">Liên hệ</a>
                                    </li>
                                </ul>
                            </nav>
                        </div>
                        <div class="button-header">
                            <a href="{{route('advise')}}" class="sc-button btn-icon">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2.25 6.75C2.25 15.034 8.966 21.75 17.25 21.75H19.5C20.0967 21.75 20.669 21.5129 21.091 21.091C21.5129 20.669 21.75 20.0967 21.75 19.5V18.128C21.75 17.612 21.399 17.162 20.898 17.037L16.475 15.931C16.035 15.821 15.573 15.986 15.302 16.348L14.332 17.641C14.05 18.017 13.563 18.183 13.122 18.021C11.4849 17.4191 9.99815 16.4686 8.76478 15.2352C7.53141 14.0018 6.58087 12.5151 5.979 10.878C5.817 10.437 5.983 9.95 6.359 9.668L7.652 8.698C8.015 8.427 8.179 7.964 8.069 7.525L6.963 3.102C6.90214 2.85869 6.76172 2.6427 6.56405 2.48834C6.36638 2.33397 6.1228 2.25008 5.872 2.25H4.5C3.90326 2.25 3.33097 2.48705 2.90901 2.90901C2.48705 3.33097 2.25 3.90326 2.25 4.5V6.75Z" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                </svg>                               
                                <span>Tư vấn</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>